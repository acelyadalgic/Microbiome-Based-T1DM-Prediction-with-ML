...
RANDOM_STATE = 42
DATA_DIR = "input_path"
OUTPUT_METRICS = "pooled_svm_pso_metrics.csv"
OUTPUT_FEATURES = "pooled_svm_pso_features.csv"
    ...
    def fitness_function(swarm_position):
        ...
        for particle in swarm_position:
            ...
            for train_idx, val_idx in inner_cv.split(X_train, y_train):
                X_tr, X_val = X_train.iloc[train_idx, selected_idx], X_train.iloc[val_idx, selected_idx]
...
def run_fold(model_name, repeat, fold, train_idx, test_idx, X_full, y_full, countries_full):
    ...
    for country in c_train.unique():
        m_tr, m_te = (c_train == country), (c_test == country)
    ...
    X_train_scaled = X_train_scaled.apply(pd.to_numeric).fillna(0)
    X_test_scaled = X_test_scaled.apply(pd.to_numeric).fillna(0)
...
for file_path in excel_files:
    ...
    print(f"\n[Model run started.] Model: {model_name}")
...
print(f"\n Model run finished. Results: {OUTPUT_METRICS}")