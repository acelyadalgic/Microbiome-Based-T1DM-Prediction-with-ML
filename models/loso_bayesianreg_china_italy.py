...
OUTPUT_DIRECTORY = "loso_bayesian_china_to_italy"
TRAIN_COUNTRY = "China"
TEST_COUNTRY = "Italy"
RANDOM_STATE = 42
...
for file_path in all_files:
    ...
    optimizer = ps.discrete.BinaryPSO(n_particles=30, dimensions=X_train_scaled.shape[1], 
    ...
    y_train_prob = best_model.predict_proba(X_train_scaled[selected_features])[:, 1]
    y_train_pred = best_model.predict(X_train_scaled[selected_features])
    ...
    y_test_prob = best_model.predict_proba(X_test_scaled[selected_features])[:, 1]
    y_test_pred = best_model.predict(X_test_scaled[selected_features])
    ...
    pd.DataFrame([summary]).to_csv(f"{OUTPUT_DIRECTORY}/loso_bayesian_summary.csv", mode='a', header=not os.path.exists(f"{OUTPUT_DIRECTORY}/loso_bayesian_summary.csv"), index=False)
